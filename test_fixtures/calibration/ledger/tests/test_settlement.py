...
from ledger.settlement import (
    ...
    summarize_import,
    summarize_import_indexed,
)

IMPLEMENTATIONS = (summarize_import, build_invoice_report, summarize_import_indexed)
...
class SettlementTests(unittest.TestCase):
    def test_documented_result(self):
        ...
        expected = (
            InvoiceSummary("A", 75, 2),
            InvoiceSummary("B", 70, 1),
    ...
    def test_case_is_preserved(self):
        for implementation in IMPLEMENTATIONS:
            self.assertEqual(
                implementation([Transaction("a", 1), Transaction("A", 2)]),
    ...
    def test_invalid_fields_and_first_error_order(self):
        cases = [
            ...
            ([Transaction("A", 2.5, True)], ValueError, "row 0: amount_minor must be an integer"),
            ([Transaction(" ", False, 1)], ValueError, "row 0: invoice_id must be nonblank text"),
        ]
    ...
    def test_seeded_differential_and_aggregate_invariants(self):
        ...
        for case in range(1000):
            rows = [
                Transaction(
                    rng.choice(["A", " B ", "東京", "é", "a"]),
                    rng.randrange(-10000, 10001),
                    bool(rng.randrange(4) == 0),
                )
            ...
            with self.subTest(case=case):
                ...
                self.assertEqual(
                    sum(item.transaction_count for item in left),
                    sum(not row.voided for row in rows),
                )
                self.assertEqual(
                    [item.invoice_id for item in left],
