...
def record(**changes):
    row = {"external_id": "r1", "invoice_id": " A ", "amount_minor": "-25",
...
class PlanningTests(unittest.TestCase):
    def test_baseline_and_active_extension(self):
        rows = [InvoiceSummary("D", 0, 1), InvoiceSummary("C", -30, 2),
                InvoiceSummary("B", 100, 1), InvoiceSummary("A", 75, 1)]
        before = tuple(rows)
        ...
        self.assertEqual(extended.lines, (("A", "deferred", 75), ("B", "ready", 100),
                                          ("C", "refund_due", -30), ("D", "balanced", 0)))
    ...
    def test_validation_empty_and_thresholds(self):
        ...
        invalid = [None, InvoiceSummary(" ", 1, 1), InvoiceSummary("A", True, 1),
                   InvoiceSummary("A", 1, 0), InvoiceSummary("A", 1, True)]
        for implementation in [plan_settlements, plan_batch_with_deferrals]:
            for item in invalid:
...
class ImportTests(unittest.TestCase):
    ...
    def test_field_boundaries_and_first_error(self):
        cases = [("external_id", " "), ("invoice_id", ""), ("amount_minor", True),
                 ("amount_minor", "+1"), ("amount_minor", "1e2"), ("amount_minor", " 2"),
                 ("amount_minor", "２"), ("amount_minor", "1.5"), ("currency", "usd"),
                 ("posted_on", "2023-02-29"), ("posted_on", "2024-2-29"), ("voided", "yes")]
        ...
        self.assertEqual(ingest_rows([record(external_id="", amount_minor="bad")]).rejected,
                         ((0, "external_id"),))
        self.assertEqual(ingest_rows([]), prepare_import([]))
        for value in [True, False, "true", "false"]:
            self.assertEqual(ingest_rows([record(voided=value)]), prepare_import([record(voided=value)]))

    def test_csv_api_shared_region_and_distinct_reports(self):
        rows = [record(invoice_id=" A, 東京 "), record(external_id="r2", amount_minor="1.2"),
    ...
    def test_adapters_enforce_acceptance_before_id_reservation(self):
        ...
        self.assertEqual([item[1] for item in report["results"]], ["rejected", "accepted", "rejected"])
        self.assertEqual(report["results"][2][2], "duplicate_id")

    def test_audit_witnesses(self):
        rows = [normalize_transaction(record(amount_minor="100")),
        ...
        self.assertEqual(assess_credit_risk([InvoiceSummary("A", 75, 2)], 50), (("A", "over_limit"),))
