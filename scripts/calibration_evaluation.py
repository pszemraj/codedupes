...
def near_best_f1(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Allow at most half a percentage point of F1 loss to favor recall."""
    floor = max(row["f1"] for row in rows) - F1_RECALL_TOLERANCE
    return [row for row in rows if row["f1"] >= floor - 1e-12]
