...
- Writers publish a complete new vector matrix before atomically switching the index to its generation, and serialize through per-shard advisory file locks, so a concurrent reader can never pair an old key map with rebuilt rows. Cache clearing reads its removal count from the same locked shard snapshot it deletes, so a writer finishing while `clear` waits is included accurately.
