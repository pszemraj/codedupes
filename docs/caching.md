...
- Writers publish a complete new vector matrix before atomically switching the index to its generation, and serialize through per-shard advisory file locks under `<cache_root>/locks/`, outside the recursively deleted shard directories. The stable lock location preserves one lock domain when clearing or LRU eviction deletes and another process recreates a shard. A concurrent reader can never pair an old key map with rebuilt rows, and cache clearing reads its removal count from the same locked shard snapshot it deletes, so a writer finishing while `clear` waits is included accurately. The next locked write attempt reclaims both temporary files and complete vector generations left unpublished by a killed writer.
