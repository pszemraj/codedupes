...
- Python `;` statement separators, trailing commas (magic trailing commas included), grouping parentheses, and plain implicit string concatenations are [formatting](polyglot-languages.md#fingerprints-and-comparison-boundaries): they no longer move the structural hash, while the token hash still sees them. A `("doc")` docstring prunes like a bare one; a concatenation with an f-string part is still walked. C, Rust, JavaScript, and TypeScript fingerprints are unchanged.
