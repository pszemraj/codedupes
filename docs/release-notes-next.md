...
- Added `check --max-duplicates <N|all>` (default `20`) and the `actionable_duplicates`, `reported_actionable_duplicates`, `truncated_by_tier`, `strict_unused`, and `hidden_only_failure` summary fields; the terminal summary gained an `Actionable duplicates` row and its `Truncated duplicates` row names the cut tiers. See [report selection](output.md#report-selection).
