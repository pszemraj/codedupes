...
- `[project.scripts]`, `[project.gui-scripts]`, and `[project.entry-points]` group entries in the nearest `pyproject.toml` at or above the scan target (a directory root or a single file's own directory), stopping after the git work-tree root so an unrelated ancestor project is never picked up; each target credits only the definition at exactly its `module:object` path (`pkg.cli:App.run` is not `Outer.App.run` or a `run` nested in a function), and a bare module or value missing `:` credits nothing
