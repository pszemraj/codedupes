...
- `test_*` definitions, definitions in pytest `conftest.py` files (fixtures and hooks pytest calls by name), and definitions in files matching the default test-file exclude shapes: a `test_*` file-name prefix, or a `_test`/`_tests` file-name stem suffix (`legacy_testament.py` does not match; `probe_test.py`, `probe_tests.py`, and `test_probe.py` do)
