...
A lowercase `.h` file is ambiguous because both C and C++ use that extension. Automatic detection treats headers as C only when the scanned tree contains at least one `.c` file and no detected C++ source/header extension. Case-sensitive `.C` and `.H` suffixes are conventional C++ spellings and are never normalized into supported C files. The detection scan prunes exactly the directories default extraction skips: C++ inside an excluded directory such as `node_modules` cannot flip the decision, while C++ in a directory the walk analyzes (including `vendor/`) disables header parsing rather than letting C++ headers be parsed with the C grammar. Explicitly selecting C also accepts lowercase `.h` headers:
