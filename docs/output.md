...
`--focus PATH` (repeatable) scopes the emitted report and the exit code to findings touching one or more paths, without changing what the analyzer scanned: `units`, `run`, and every diagnostics array stay corpus-wide, so the underlying scan is identical with or without `--focus`. Only `duplicates`, `exact_families`, `potentially_unused`, and the counts derived from them shrink to the focus scope; `total_units` and `units` still reflect the full corpus, minus units referenced only by findings the focus dropped. Each focus path must exist and resolve inside the scan root, and the target itself must be a directory — a file target with `--focus` is a usage error naming the root to scan instead.
