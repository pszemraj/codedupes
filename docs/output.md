...
The exit code is computed on the focused result, so `--focus` narrows what `--fail-on`/`--fail-on-incomplete` can fail on to the scoped findings; a run that fails unfocused can pass focused, and vice versa. `--focus` composes with `--unused-only` and the other detection modes: it filters whichever finding lists that mode produces.
