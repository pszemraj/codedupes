...
The opt-in guard exists because the positive path is unvalidated: the gate proves the CPU can execute bf16 GEMM fast, not that the float32-calibrated duplicate and search thresholds survive bfloat16's numeric shift on the built-in models. Automatic enablement waits for a gate-passing machine (AMX/AVX512-bf16 x86, or ARM bf16 with an mkldnn backend) to validate speed and decision parity end to end.
...
The gate's verdict is cached on disk under `<cache_root>/machines/<digest>.json`, where the digest identifies this host and Python/torch environment (platform system/release/arch, hostname, Python executable and prefix, torch version and install location, plus a record schema), so opted-in repeat runs never re-import torch merely to derive a cache key. A cache directory that travels across machines, conda envs, or same-version torch reinstalls re-probes per environment instead of trusting a foreign verdict, and each record stores the raw ISA and mkldnn values behind its decision. The identity is environment metadata, not a CPU microarchitecture fingerprint: hosts deliberately built to identical images with pinned hostnames share one record, which is why the record stays a cache of a re-derivable probe, never a source of truth. The directory is bounded (oldest records pruned past a small cap, for environments whose identity churns per run) and removed by a full `codedupes cache clear`. A run without the opt-in never consults the record at all, and a run with `--no-cache` or `CODEDUPES_NO_CACHE` set never reads or writes it either.
