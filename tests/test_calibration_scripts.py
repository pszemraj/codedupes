...
def corpus_path(tmp_path: Path) -> Path:
    """Write a four-file corpus with an exact pair and a labeled near pair."""
...
def _run_validator(monkeypatch: pytest.MonkeyPatch, corpus_path: Path, labels: dict) -> int:
    ...
    :param dict labels: Labels to serialize beside the corpus.
    :return int: Validator exit code.
    """
    labels_path = corpus_path.parent / "labels.json"
...
def test_validator_reports_every_failure_when_a_label_spec_cannot_resolve(
    corpus_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    ...
    exit_code = _run_validator(monkeypatch, corpus_path, labels)
...
def test_validator_counts_negative_controls_as_groups(
    ...
    labels = {
        ...
        "negative_controls": [["gamma.py::summarize", "delta.py::tally", "alpha.py::accumulate"]],
    }

    _run_validator(monkeypatch, corpus_path, labels)
