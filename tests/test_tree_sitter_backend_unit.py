...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "# codedupes: ignore generated by protoc",
            {"unused", "duplicates"},
            set(),
            id="bare-with-reason",
        ),
        ...
        pytest.param(
            "/* codedupes: ignore*/", {"unused", "duplicates"}, set(), id="block-comment-tight"
        ),
        pytest.param(
            "/* codedupes: ignore\n[details]\n*/",
            {"unused", "duplicates"},
...
@pytest.mark.parametrize(
    ("text", "message"),
    [
        pytest.param("# codedupes: ignore[unused", "Unclosed suppression kind list", id="unclosed"),
        pytest.param("# codedupes: ignore-me", "Unexpected '-'", id="hyphen-suffix"),
        pytest.param("// codedupes: ignore:", "Unexpected ':'", id="colon-suffix"),
        pytest.param("# codedupes: ignore.", "Unexpected '.'", id="period-suffix"),
    ],
)
def test_parse_suppressions_rejects_malformed_directives(text: str, message: str) -> None:
    """A directive that is neither bare nor a closed kind list is an error, not suppress-all."""
